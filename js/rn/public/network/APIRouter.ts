...
class APIRouter {
  ...
  static activity_redBagDetail = () => {
    return (
      httpClient.get<RedBagDetailActivityModel>('c=activity&a=redBagDetail')
    )
  }
  ...
  static user_login = (
    ...
    return (
      httpClient.post<
        LoginModel>
        ('c=user&a=login',
          { usr: uname, pwd: pwd, ggCode: googleCode, ...slideCode })
    )
  }
  static user_balance_token = async () => {
    const user = await OCHelper.call('UGUserModel.currentUser')
    return (
      httpClient.get<BalanceModel>('c=user&a=balance&token=' + user.token)
    )
  }
