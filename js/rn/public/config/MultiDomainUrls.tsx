 ...
 */
const MultiDomainUrls: {[x:string]:[string]} = {
  'zora': [
...
const initDomain = async (siteId?: string) => {
  switch (Platform.OS) {
    case 'ios':
      const host = MultiDomainUrls[siteId][0];
      host.length && OCHelper.call('AppDefine.shared.setHost:', [host]);
      break;
    case 'android':
      await ANHelper.callAsync(CMD.INIT_DOMAIN, MultiDomainUrls);
      break;
  }
}

export {MultiDomainUrls, initDomain};
