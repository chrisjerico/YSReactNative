...
export default class PromotionItem extends Component<IProps, IState> {
    ...
    let contentView = (
      <View
        style={{
          ...
          marginVertical: AppDefine.siteId == 'c092' ? 10 : marginVertical,
          backgroundColor: AppDefine.siteId == 'c092' ? '#e0e0e0' : '#00000000',
          borderRadius: 8,
        }}>
        <TouchableOpacity
          activeOpacity={1}
        ...
        {this.state.selectedIndex === idx && (
          <View style={{ height: pm.webViewHeight ?? 200 }}>
            <WebView
  ...
  render() {
    if (!this.list.length) {
      return <Text style={{ marginTop: 50, textAlign: 'center', color: 'white' }}>暂无</Text>
    }
    if (this.style1 == '外边框') {
      return (
        <View style={{ margin: 7, paddingTop: 11, borderColor: 'white', borderWidth: 1, borderRadius: 7, backgroundColor: Skin1.homeContentColor }}>
          <FlatList
            ...
            data={this.list}
            renderItem={(data) => this.renderCell(data.item, data.index)}
