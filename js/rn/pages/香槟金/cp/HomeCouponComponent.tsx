...
interface IProps {
  reducerData: ICouponBean;
}

/**
 * 主页优惠活动
 */
export default class HomeCouponComponent extends Component<IProps> {
  /**
   * 绘制优惠活动
   *
   * @private
  ...
  _renderCoupon(): React.ReactNode {
    const coupon = this.props.reducerData;
    ...
    return (
      <View key="_renderCoupon">
        ...
        {coupon.list.map((item, index) => (
          <View key={index} style={[_styles.couponItemContainer, {backgroundColor: UGColor.placeholderColor2}]}>
