...
import { CqsscCode, HcmTabIndex, LCode, LhcCode } from '../../../const/LotteryConst'
  ...
  playOddData?.pageData?.groupTri?.map((pageData, index) => {
    ...
    pageData?.map((groupData, index) => {
      ...
      if (gameType == LCode.cqssc && playOddData?.code == CqsscCode.WX && groupData?.alias == '单式') {//秒秒彩五行单式特殊处理
        ...
        if (!anyEmpty(groupData?.allHcPlays) && UGStore.globalProps?.fastTabIndex == HcmTabIndex.快速选择) {//越南彩里面会有这种 快速选择 的彩种
