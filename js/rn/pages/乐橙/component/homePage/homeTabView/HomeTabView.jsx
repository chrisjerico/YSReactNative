...
exports.HomeTabView = function () {
    ...
    var getTab = function (item, index) {
        return index == 0 ?
            <RecommendTabView_1.RecommendTabView onPress={onPress} list={item.list} tabLabel={item.name}/> :
            index == 1 ?
                <LotteryTabView_1.LotteryTabView onPress={onPress} list={item.list} tabLabel={item.name}/> :
                <GameListView_1.GameListView list={item.list} onPress={onPress} tabLabel={item.name}/>;
    };
    ...
    }} tabBarUnderlineStyle={{ height: 2, backgroundColor: "#3c3c3c" }} tabBarTextStyle={{ color: "#3c3c3c" }} style={[{ marginHorizontal: 10, backgroundColor: "#ffffff", borderRadius: 10, height: height }]} renderTabBar={function (props) { return (<react_native_1.ScrollView horizontal={true} showsHorizontalScrollIndicator={false} bounces={false} style={{ flexDirection: 'row', paddingBottom: 10 }}>
