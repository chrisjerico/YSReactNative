...
exports.HomeTabView = function () {
    ...
    var getTab = function (item, index) {
        return index == 0 ?
            <RecommendTabView_1.RecommendTabView banner={banner} list={item.list} marquee={marquee} onlineNum={onlineNum} tabLabel="精选"/> :
            item.name.indexOf("彩票") != -1 ?
                <LotteryTabView_1.LotteryTabView list={item.list} tabLabel="彩票"/> :
                <GameListView_1.GameListView list={item.list} tabLabel={item.name}/>;
    };
    ...
    return (<>
            {(games === null || games === void 0 ? void 0 : games.length) > 0 &&
        <react_native_scrollable_tab_view_1.default onChangeTab={function (_a) {
            var i = _a.i;
            return calculateHeight(i);
        }} tabBarUnderlineStyle={{ height: 2, backgroundColor: "red" }} tabBarTextStyle={{ color: "#666666", fontWeight: "bold" }} style={[{ flex: 1, height: height }]} renderTabBar={function () { return <react_native_scrollable_tab_view_1.ScrollableTabBar style={{ backgroundColor: "#ffffff" }}/>; }}>
